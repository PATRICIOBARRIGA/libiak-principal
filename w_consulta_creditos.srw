HA$PBExportHeader$w_consulta_creditos.srw
$PBExportComments$Si.Consulta Ordenes de compra, notas de recepci$$HEX1$$f300$$ENDHEX$$n, y compras
forward
global type w_consulta_creditos from window
end type
type cb_1 from commandbutton within w_consulta_creditos
end type
type st_dw_mode from statictext within w_consulta_creditos
end type
type cb_action from commandbutton within w_consulta_creditos
end type
type dw_1 from datawindow within w_consulta_creditos
end type
end forward
...
global w_consulta_creditos w_consulta_creditos

type variables
datawindowchild   idwc_tipo,idwc_refpagos
boolean  ib_flag
end variables
forward prototypes
...
on w_consulta_creditos.destroy
destroy(this.cb_1)
...
event rowfocuschanged;String  ls_sucodigo,ls_eccodigo,ls_pvcodigo,ls_cofacpro
Long    ll_numero
...
ls_cofacpro    =  This.GetItemString(currentrow,"co_facpro")
//
//
...
if dwo.name = 'co_facpro' then
dw_1.GetChild('co_facpro',idwc_refpagos)
idwc_refpagos.SetTransObject(sqlca)
idwc_refpagos.retrieve(this.Object.mp_codigo[row],this.Object.pv_codigo[row])
