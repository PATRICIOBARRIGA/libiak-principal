HA$PBExportHeader$w_recepcion_pedido.srw
$PBExportComments$Si.Recepciones de pedido de una sucursal
forward
global type w_recepcion_pedido from w_sheet_master_detail
end type
type dw_ubica from datawindow within w_recepcion_pedido
end type
type dw_movim from datawindow within w_recepcion_pedido
end type
type shl_1 from statichyperlink within w_recepcion_pedido
end type
type dw_1 from datawindow within w_recepcion_pedido
end type
end forward
...
global w_recepcion_pedido w_recepcion_pedido
...
int      max,i=0,rc,li_i
datetime now
long    ll_numero, ll_secue,ll_totalreg,ll_i,ll_num_nota
decimal  lc_sumsaldo, ld_saldo
string   ls_val ='S', ls_valstk, ls, ls_numero, ls_null, ls_kit
string ls_codigo,ls_atomo
dec{2} lc_stock,lc_cantid,lc_pedido,lc_cantatomo
dec{6} lc_costo_atomo
char lch_kit
...
for i = 1 to max
		...
		if f_carga_stock_bodega_new(str_appgeninfo.seccion,ls_codigo,lc_cantid,lch_kit,ls_atomo,lc_cantatomo) = false Then
			...
			return
		End if			
	   if wf_mov_inventario(ls_codigo,lc_cantid,now,ll_num_nota,&
...
event ue_anular;// Descripci$$HEX1$$f300$$ENDHEX$$n : Llama a la funci$$HEX1$$f300$$ENDHEX$$n Anula_nota_de_recepci$$HEX1$$f300$$ENDHEX$$n que elimina de
...
type dw_master from w_sheet_master_detail`dw_master within w_recepcion_pedido
integer x = 0
integer y = 0
integer width = 2578
integer height = 644
...
type dw_detail from w_sheet_master_detail`dw_detail within w_recepcion_pedido
...
integer width = 2578
integer height = 828
string dataobject = "d_detalle_recepcion_pedido"
...
type dw_report from w_sheet_master_detail`dw_report within w_recepcion_pedido
integer x = 0
integer y = 0
integer width = 2587
integer height = 1296
...
string text = "Click aqu$$HEX2$$ed002000$$ENDHEX$$para importar Archivo"
...
str_prodparam.ventana = parent
str_prodparam.datawindow = dw_detail
str_prodparam.fila = dw_detail.GetRow() 
str_prodparam.campo = "dc_cantid"
cantidad_producto_ubica = TRUE
...
//			end if	
			...
			into :ls_codigo,:ls_nombre,:lch_kit
			from in_item
			where em_codigo = :str_appgeninfo.empresa
			...
			dw_detail.setitem(ll_i,"em_codigo",str_appgeninfo.empresa)
			dw_detail.setitem(ll_i,"su_codigo",str_appgeninfo.sucursal)
			dw_detail.setitem(ll_i,"bo_codigo",str_appgeninfo.seccion)
			
			dw_detail.setitem(ll_i,"dc_secue",ll_i)		
						
			dw_detail.setitem(ll_i,"it_codigo",ls_codigo)
			dw_detail.setitem(ll_i,"codant",ls_codant)
			dw_detail.setitem(ll_i,"nombre",ls_nombre)
						
			dw_detail.setitem(ll_i,"dc_cantid",ll_cantid)
       		dw_detail.setitem(ll_i, 'it_kit', lch_kit)
		//	dw_detail.setitem(ll_i,"dc_costo" ,lc_costo)
    next	
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_importa_items_recepcion"
boolean livescroll = true
borderstyle borderstyle = styleraised!
end type
