HA$PBExportHeader$w_rep_cierre_caja.srw
$PBExportComments$si
forward
global type w_rep_cierre_caja from w_sheet_1_dw
end type
type dw_sel_rep from datawindow within w_rep_cierre_caja
end type
type dw_cc from uo_dw_comprobante within w_rep_cierre_caja
end type
type dw_sd from datawindow within w_rep_cierre_caja
end type
type cb_1 from commandbutton within w_rep_cierre_caja
end type
type dw_1 from datawindow within w_rep_cierre_caja
end type
end forward
...
global w_rep_cierre_caja w_rep_cierre_caja

type variables
string is_numero,is_cajero,is_caja
date id_fecha
...
if isnull(is_caja) then
		messageBox('Por Favor','Seleccione una caja para obtener el reporte')
		return -1
end if	
...
dw_sel_rep.Object.cajero_t.color = rgb(128,128,128)

dw_sel_rep.Object.cajero.protect = 1
dw_sel_rep.Object.cajero.background.Color = rgb(245,245,245)
...
type dw_datos from w_sheet_1_dw`dw_datos within w_rep_cierre_caja
integer y = 332
integer width = 3493
integer height = 1872
...
type dw_report from w_sheet_1_dw`dw_report within w_rep_cierre_caja
integer x = 5
integer y = 364
integer width = 741
integer height = 260
...
char    lch_tipo
decimal lc_cheq_pag=0,lc_deposito = 0,lc_efectivo= 0,lc_totalventa = 0
dec{2}  lc_totrec= 0,lc_cambio= 0,lc_chdif = 0,lc_falta =0,lc_diferencia= 0,lc_ret_pag =0
long    ll_nreg
...
If dwo.name = 'b_cierre' Then
    ...
    //Valor del cambio otorgado 
	select sum(nvl(ve_cambio,0))
	into :lc_cambio
	from fa_venta
	...
	es_codigo = 2 and
	trunc(ve_fecha) = :id_fecha and
	cj_caja = to_number(:is_caja);
	if isnull(lc_cambio) then lc_cambio = 0


	dw_datos.setredraw(false)	
	ll_nreg = dw_datos.Retrieve(integer(str_appgeninfo.sucursal),integer(str_appgeninfo.seccion),is_caja,id_fecha,id_fecha,lc_cambio)
	If ll_nreg < 1 Then 
	...
	//Inicia el calculo de valores para el cierre
	...
	//Pintar calculos en el documento para impresion del cierre
