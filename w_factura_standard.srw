HA$PBExportHeader$w_factura_standard.srw
$PBExportComments$Respaldo de w_factura_standard
forward
global type w_factura_standard from w_sheet_master_detail
end type
type cb_ubicacion from commandbutton within w_factura_standard
end type
type cb_1 from commandbutton within w_factura_standard
end type
type cb_2 from commandbutton within w_factura_standard
end type
type cb_3 from commandbutton within w_factura_standard
end type
type cb_formpag from commandbutton within w_factura_standard
end type
type dw_sel_numero from datawindow within w_factura_standard
end type
type p_ubicacion from picture within w_factura_standard
end type
type dw_movim from datawindow within w_factura_standard
end type
type dw_consulta_stk from datawindow within w_factura_standard
end type
type p_producto from picture within w_factura_standard
end type
type sle_1 from singlelineedit within w_factura_standard
end type
type cb_4 from commandbutton within w_factura_standard
end type
type pb_1 from picturebutton within w_factura_standard
end type
type st_1 from statictext within w_factura_standard
end type
type st_plazo from statictext within w_factura_standard
end type
type st_2 from statictext within w_factura_standard
end type
type cb_5 from commandbutton within w_factura_standard
end type
type dw_1 from datawindow within w_factura_standard
end type
type dw_pedidos from datawindow within w_factura_standard
end type
type dw_tinte from datawindow within w_factura_standard
end type
type dw_detalle_pago from uo_dw_detail within w_factura_standard
end type
end forward
...
integer width = 7488
integer height = 1972
string title = "Factura Standard 20.09.2K16 con SP - IVA"
...
global w_factura_standard w_factura_standard

type variables
 dec{2}  ic_iva,ic_descuento,ic_precio,ic_desc1,ic_desc2,ic_desc3
dec{4}  ic_costo
int     il_descue,ii_excento_iva,ii_numli
string  is_mensaje, is_estado='1',is_codant1,is_numpedido,is_unidad
...
//Pintamax
//dw_report.DataObject = 'dw_factura_pintamax_a4'
//dw_report.DataObject = 'dw_factura_redcolor_fpos'
//dw_report.DataObject = 'dw_factura_megaherrajes'

// Redcolor locales y pinturasexpress
dw_report.DataObject = 'd_nventa_new' 
dw_report.SetTransObject(sqlca)
...
//dw_report.retrieve(is_estado,str_appgeninfo.empresa,str_appgeninfo.sucursal,str_appgeninfo.seccion, ll_ve_numero,0.12)
//fin de pintamax
...
integer li_i,li_j,li_plazo, li_plazocli, fila,li_filact

li_filact = dw_master.GetRow()

ls_cliente = dw_master.GetItemString(li_filact,'ce_codigo')
li_plazocli = dw_master.GetItemNumber(li_filact,"fa_clien_ce_plazo")
lch_estcre = dw_master.GetItemString(li_filact,"fa_clien_ce_estcre")
lch_facturar = dw_master.GetItemString(li_filact,"fa_clien_ce_facturar")
lc_salcre =  dw_master.GetItemNumber(li_filact,"fa_clien_ce_salcre")

//Poner como variables de instancia,el cliente ya ha sido seleccionado antes de llegar a esta instancia,
//Deberia tener todo los datos listos.
...
for li_i = 1 to dw_detalle_pago.RowCount()
	...
	li_plazo    = dw_detalle_pago.GetItemNumber(li_i,'plazo_fp')
	ls_fpvehiculo = dw_detalle_pago.GetItemString(li_i,'fp_acumula')
	
	//Asignar al momento de escoger la forma de pago, evitar el SELECT por c/fp
...
//	end if
	...
	lc_aux = dw_detail.getitemnumber(1,"cc_desc2")
...
do
	fetch c2 into :str_detalle[li].item,:str_detalle[li].secue,:str_detalle[li].cantidad,
	              :str_detalle[li].precio,:str_detalle[li].desc1,:str_detalle[li].desc2,&
					  :str_detalle[li].desc3,:str_detalle[li].descue,:str_detalle[li].total,:str_detalle[li].dv_motor,:str_detalle[li].dv_chasis;
...
for li = 1 to ll_num_filas
	...
	if ls_valstk = 'S' then
		If f_dame_stock_bodega_new(str_appgeninfo.seccion,str_detalle[li].item,lch_kit,lc) = false then
			...
			dw_detail.SetItem(li,"dv_motor",str_detalle[li].dv_motor)  //formula
			dw_detail.SetItem(li,"dv_chasis",str_detalle[li].dv_chasis)  //presentacion
...
for i = 1 to num_filas
	...
	ls_valstk = dw_detail.getitemstring(i,"it_valstk")
	ls_kit = dw_detail.getitemString(i,"it_kit")
	lc_cantidad = dw_detail.getitemdecimal(i,"dv_candes")	
	lc_candes = 0
	
 	If lc_cantidad <= 0 then return -1
...
If ll_filact = 0 Then
	...
	lc_iva = dw_detail.getitemdecimal(ll_filact,"cc_iva")
...
this.Control[iCurrent+15]=this.st_plazo
this.Control[iCurrent+16]=this.st_2
this.Control[iCurrent+17]=this.cb_5
this.Control[iCurrent+18]=this.dw_1
this.Control[iCurrent+19]=this.dw_pedidos
this.Control[iCurrent+20]=this.dw_tinte
this.Control[iCurrent+21]=this.dw_detalle_pago
end on

on w_factura_standard.destroy
call super::destroy
...
select sysdate  into :id_hoy from dual;
...
string    ls_fp,ls_nc,ls_cliente,ls_codigo,ls_insfin,ls_cuenta,&
			 ls_numche,ls_numnd,ls_numero,ls_codant,ls_atomo,ls_marca, ls_motor, ls_numfact,ls_errmsg,ls_numserie
...
select cj_ticket,serie
into :ll_factura,:ls_numserie
from fa_caja
where em_codigo = :str_appgeninfo.empresa
and su_codigo = :str_appgeninfo.sucursal
...
//		If ll_candes > 0 Then
//			  //DECLARE SP_MOV_INV PROCEDURE FOR
//		End if
 next	
	 ...
	 if rc = 1 then
	    rc = dw_detalle_pago.update(true,false)
		 if rc = 1 then
				 ...
				 //Ingresar mov de inventario
				 DECLARE SP_INVMOV PROCEDURE FOR SP_INV_MOV_INVENTARIO (:str_appgeninfo.empresa,:str_appgeninfo.sucursal,:str_appgeninfo.seccion,'2',:is_estado,:ll_factura) ;
				 EXECUTE SP_INVMOV;
				 if SQLCA.SqlCode < 0  then
					 Messagebox("Atencion","Problemas al ejecutar SP_INV_MOV_INVENTARIO"+ SQLCA.sqlerrtext)
					 Rollback;
					CLOSE SP_INVMOV;
					RETURN 
					end if	
				...
				//Inicia el descargo de tintes
				 DECLARE SP_INVTINTE PROCEDURE FOR SP_INV_DESCARGA_TINTES (:str_appgeninfo.empresa,:str_appgeninfo.sucursal,:str_appgeninfo.seccion,'2',:is_estado,:ll_factura) ;
				 EXECUTE SP_INVTINTE; 
				 if SQLCA.SqlCode < 0 then
				Messagebox("Atencion","Problemas al ejecutar SP_INV_DESCARGA_TINTES"+ SQLCA.sqlerrtext)
				Rollback;
				CLOSE SP_INVTINTE;
				RETURN
				end if	
				CLOSE SP_INVTINTE;
				 ...
				 //Registrar cuenta por cobrar y registro de pago,
				 DECLARE SP_CXC PROCEDURE FOR SP_CXC_CUENTA_X_COBRAR(:str_appgeninfo.empresa,:str_appgeninfo.sucursal,:str_appgeninfo.seccion,:ll_factura,:is_estado, :str_appgeninfo.caja) ;
				 EXECUTE SP_CXC;
				 if SQLCA.SqlCode < 0 then
					Rollback;
					Messagebox("Atencion","Problemas al  Ejecutar en el SP_CXC_CUENTA_X_COBRAR"+ SQLCA.sqlerrtext)
					CLOSE SP_CXC;
					RETURN 
				 else
					FETCH SP_CXC INTO :ls_errmsg;
					CLOSE SP_CXC;
				 end if	
...
type dw_master from w_sheet_master_detail`dw_master within w_factura_standard
integer x = 0
integer y = 0
integer width = 7410
integer height = 748
...
long ll_filact,ll_numero,ll_plazo
decimal{2} lc_ivavalor,lc_valor,lc_subtot, lc_descue, lc_cargo, lc_valpag,lc_cupcre,lc_salcre
string ls,ls_nomcli, ls_cliente, ls_filtro, ls_vendedor, ls_firma, ls_datos[],ls_tccodigo,ls_column,ls_actividad,ls_facturar,ls_estcre
char lch_cambven
...
case  'ce_codigo' 
	...
	select ep_codigo, ce_cambven, ce_exiva, ce_firma,tc_codigo,ce_actividad,ltrim(decode(ce_razons,null,ce_nombre||'  '||ce_apelli,ce_razons||' '||ce_nomrep||' '||ce_aperep)) , ce_cupcre,ce_salcre,nvl(ce_facturar,'S')  ,ce_estcre,ce_plazo
	into :ls_vendedor, :lch_cambven,:ls, :ls_firma,:ls_tccodigo,:ls_actividad,:ls_nomcli,:lc_cupcre,:lc_salcre,:ls_facturar,:ls_estcre,:ll_plazo
	from fa_clien
	where em_codigo = :str_appgeninfo.empresa
	and    ce_codigo= :ls_cliente;

  
     this.setitem(row,"fa_clien_ce_facturar",ls_facturar) 
     this.setitem(row,"fa_clien_ce_cupcre",lc_cupcre)
	 this.setitem(row,"fa_clien_ce_salcre",lc_salcre)
	 this.setitem(row,"fa_clien_ce_estcre",ls_estcre)
     this.setitem(row,"fa_clien_ce_plazo",ll_plazo)
...
//	is_mensaje = 'Cliente no registrado'
	...
	//if wf_valida_cliente(ls_cliente,'ce_facturar') <> 1 then
	 ...
	 if ls_facturar = 'N' or ls_facturar = "" then
	...
	//if wf_valida_cliente(ls_cliente,'ce_estcre') <> 1 then
	 ...
	 if  ls_estcre <> 'A' then
		//is_mensaje = 'No se puede facturar a este cliente'
		Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","No se puede facturar a este cliente, verifique el estado del cr$$HEX1$$e900$$ENDHEX$$dito.")
		sle_1.text = ""
		dw_detail.enabled = false
		ib_nograba = true
		return 1
...
event dw_master::updatestart;call super::updatestart;LONG ll_row,ll_rowm,i,ll_plazo
decimal lc_subtotal,ld_valor,lc_iva,lc_cargo,lc_valpag,lc_subtot_tarifa_X
gb_codigo_numerico = true
String ls_leyenda,ls_aux
Date ld_fecha
/**/
...
//Obtener del detalle valores para actualizar la cabecera con info real
lc_cargo = dw_master.GetItemDecimal(ll_rowm,"ve_cargo")
lc_subtotal = dw_detail.GetItemDecimal(1,"cc_sum_subtot")
lc_subtot_tarifa_X = dw_detail.GetitemDecimal(1,"cc_totiva")
lc_iva = dw_detail.GetItemDecimal(1,"cc_iva")

//Validar que el iva corresponda al neto 
//if  round(lc_iva,2) <> round(lc_subtot_tarifa_X*f_iva(),2) then
//	Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","El valor de iva no coincide con el calculado para la base imponible...!!!")
//	return 1
//end if
...
//Actualiza cabecera
dw_master.SetItem(ll_rowm,"ve_subtot",lc_subtotal)
dw_master.SetItem(ll_rowm,"ve_iva",lc_iva)
lc_valpag = lc_subtotal + lc_iva +lc_cargo
dw_master.SetItem(ll_rowm,"ve_valpag",lc_valpag)
ld_fecha = date(dw_master.getItemDateTime(ll_rowm, "ve_fecha"))


//Actualiza la cabecera con el detalle del pago
for i = 1 to dw_detalle_pago.rowcount()
	ll_plazo = dw_detalle_pago.object.plazo_fp[ i ]
	ld_valor = dw_detalle_pago.object.rp_valor[ i ]
		If ll_plazo = 0 then
		...
		End if
next
dw_master.setitem(dw_master.getrow(),"ve_leyenda",ls_leyenda)
return 0
end event
...
type dw_detail from w_sheet_master_detail`dw_detail within w_factura_standard
...
event dw_detail::ue_darimagen;int li_fila, li
string ls_produ, ls
...
decimal{2} lc_prefab,lc_pedido, lc_stock,ld_null,lc_recargo,lc_preciodesc,lc_descprom
dec{4}     lc_costo
string    ls_columnName ,ls,ls_marca,ls_pepa, ls_nombre, ls_null, ls_codant,ls_tipo_desc,&
			tipo_descuento,ls_base,ls_param[2],ls_actividad,ls_itcodigo,ls_fp,ls_medida,lch_modifica,ls_presentacion
char       lch_cambia,lch_kit, lch_aux,ls_valstk
decimal    ld_desc1, ld_desc2, ld_desc3
...
//	DECLARE SP_ITEM PROCEDURE FOR SP_INV_ITEMS (:str_appgeninfo.empresa,:ls);
//	EXECUTE SP_ITEM;
//    if SQLCA.SqlCode < 0 then
//    Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","Problemas al ejecutar el SP_INV_ITEMS"+ SQLCA.sqlerrtext)
//	CLOSE SP_ITEM;
//	RETURN -1
//	end if	
//	
//	
//	
//     FETCH SP_ITEM INTO :ls_marca,:li_ivaitem,:lch_cambia,:ls_pepa,:ls_codant,:ls_nombre,:lc_prefab, :ls_valstk, :ic_costo, :lch_kit, :ls_base,:ls_medida,:ls_presentacion;
//	If SQLCA.SqlCode < 0 then
//     Messagebox("Atencion","Problemas al seleccionar el item...FETCH"+ SQLCA.sqlerrtext)
//	CLOSE SP_ITEM;
//	RETURN -1
//	end if	
//	CLOSE SP_ITEM;
...
select ma_codigo,it_tiemsec,it_codigo, it_nombre, it_prefab, it_valstk,it_kit,it_costo,it_codant,it_base,ub_codigo,decode(ub_codigo,3,'CANECA',6,'GALON',7,'LITRO',8,'OCTAVO',9,'DIECISEIS',11,'TREINTADOS')
into :ls_marca,:li_ivaitem,:ls_pepa, :ls_nombre, :lc_prefab, :ls_valstk,:lch_kit,:ic_costo,:ls_codant,:ls_base,:ls_medida,:ls_presentacion
from in_item
where em_codigo = :str_appgeninfo.empresa
and it_codant = :ls;
//--and (it_codant = :ls or  it_codbar = :ls);	


 If isnull(ls_pepa) or ls_pepa = '' then
  ...
  this.SetItem(li_fila,"it_valstk",ls_valstk)
  this.SetItem(li_fila,"ma_codigo",ls_marca)
  this.SetItem(li_fila,"it_preparado",lch_cambia)
  this.setitem(li_fila, 'dv_motor',ls_null)
...
//            	DECLARE SP_INV_STOCK_X_SUC PROCEDURE FOR SP_INV_DAME_STOCK_SUCURSAL (:str_appgeninfo.empresa,:str_appgeninfo.sucursal,:ls_pepa,:lch_kit,:lc_pedido );
//		     EXECUTE SP_INV_STOCK_X_SUC;
//			FETCH SP_INV_STOCK_X_SUC INTO :lc_stock,:lb_flag;
//               CLOSE SP_INV_STOCK_X_SUC;

//		  If f_dame_stock_sucursal_new(ls_pepa,lc_pedido,lc_stock,lch_kit ) = false then
           If f_dame_stock_sucursal(ls_pepa,lc_pedido,lc_stock,lch_kit,lc_recargo ) = false then
...
//		
		...
		//Pinta datos de items que no validan stock
		this.setitem(li_fila, 'dv_cantid', 1)
		this.setitem(li_fila, 'dv_candes', 1)
		this.setitem(li_fila, 'nombre',ls_nombre)
		this.setitem(li_fila, 'dv_precio', lc_prefab)
...
case  'dv_cantid'
	...
	lch_kit = this.GetItemString(li_fila,'it_kit')
	ls_pepa = this.GetItemString(li_fila,'it_codigo')
	ls_valstk = this.GetitemString(li_fila,'it_valstk')
	...
	if ls_valstk = 'S' then
			DECLARE SP_INV_STOCK_X_SUC1 PROCEDURE FOR SP_INV_DAME_STOCK_SUCURSAL (:str_appgeninfo.empresa,:str_appgeninfo.sucursal,:ls_pepa,:lch_kit,:lc_pedido);
		     EXECUTE SP_INV_STOCK_X_SUC1;
			FETCH SP_INV_STOCK_X_SUC1 INTO :lc_stock,:lb_flag;
			CLOSE SP_INV_STOCK_X_SUC1;
...
If dwo.name = "dv_desc3" Then 
   ...
   lch_cambia =  this.GetitemString(row,"it_preparado")
...
lch_kit = this.GetItemString(currentrow,"it_kit")
lch_cambia = this.GetItemString(currentrow,"it_preparado")
...
event dw_detail::buttonclicked;call super::buttonclicked;String ls_pepa,lch_modifica,ls_nulo,ls_base,ls_medida
Decimal{2} lc_precio
...
lch_modifica = message.StringParm
if lch_modifica = 'S' then
	this.setitem(row,'dv_motor',str_prodparam.formula)
	this.setitem(row,'dv_precio',str_prodparam.precio)	
	 if wf_cambia_precio_desc() = -1 Then 
		         this.SetItem(getrow(),'dv_precio',ic_precio)	
	end if
	wf_encera_pago()
...
type dw_report from w_sheet_master_detail`dw_report within w_factura_standard
integer x = 105
integer y = 4
integer width = 2784
integer height = 732
integer taborder = 0
string dataobject = "d_nventa_new_pinturasexpress"
...
//Variables para todo el evento
 ls_cliente = dw_master.GetItemString(ll_filcab,'ce_codigo')
li_plazocli = dw_master.GetItemNumber(ll_filcab,'fa_clien_ce_plazo')
lch_estcre = dw_master.GetItemString(ll_filcab,'fa_clien_ce_estcre')
lc_salcre = dw_master.GetItemNumber(ll_filcab,'fa_clien_ce_salcre')
//
...
if dwo.name = 'fp_codigo' then
	 ...
	 select fp_plazo,fp_acumula,fp_numpag
	 into :li_plazo,:ls_acumula,:li_numcuota
	 from fa_formpag 
	 where em_codigo = :str_appgeninfo.empresa
	 and fp_codigo = :ls_codigo
 	 and fp_utiliza = 'S';
	 
	 this.SetItem(row,"plazo_fp",li_plazo)
	 this.SetItem(row,"fp_acumula",ls_acumula)
	 this.SetItem(row,"num_cuotas",li_numcuota)
	 
	lc_descuento = dw_detail.getitemnumber(1,"cc_desc2")
    ...
    //Quitar de aqui , el cliente en esta instancia ya debe cargar todos los datos del credito
...
//	end if
	...
	for li_i = 1 to li_max
		accepttext()
		ls_codigo = this.GetItemString(row,'fp_codigo')
		lc_valor = this.GetItemdecimal(row,'rp_valor')
	     li_plazo = this.GetItemNumber(row,"plazo_fp")
...
//			next
		...
		if ls_codigo = '0' then
			lc_efectivo += this.getItemdecimal(row,"rp_valor")
		else
			lc_valotras += this.getItemdecimal(row,"rp_valor")
		end if
	next
...
if dwo.name = 'rp_valor' then
	...
	li_plazo        = this.GetItemNumber(row,"plazo_fp")
	ls_acumula   =  this.GetItemString(row,"fp_acumula")
	li_numcuota = this.GetItemNumber(row,"num_cuota") 
