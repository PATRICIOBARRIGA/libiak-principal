HA$PBExportHeader$w_factura_std.srw
$PBExportComments$Facturas al por mayor
forward
global type w_factura_std from w_sheet_master_detail
end type
type cb_ubicacion from commandbutton within w_factura_std
end type
type cb_1 from commandbutton within w_factura_std
end type
type cb_2 from commandbutton within w_factura_std
end type
type cb_3 from commandbutton within w_factura_std
end type
type cb_formpag from commandbutton within w_factura_std
end type
type p_ubicacion from picture within w_factura_std
end type
type dw_movim from datawindow within w_factura_std
end type
type dw_consulta_stk from datawindow within w_factura_std
end type
type p_producto from picture within w_factura_std
end type
type sle_1 from singlelineedit within w_factura_std
end type
type cb_4 from commandbutton within w_factura_std
end type
type pb_1 from picturebutton within w_factura_std
end type
type st_1 from statictext within w_factura_std
end type
type st_plazo from statictext within w_factura_std
end type
type st_2 from statictext within w_factura_std
end type
type cb_5 from commandbutton within w_factura_std
end type
type dw_1 from datawindow within w_factura_std
end type
type dw_pedidos from datawindow within w_factura_std
end type
type dw_tinte from datawindow within w_factura_std
end type
type dw_detalle_pago from uo_dw_detail within w_factura_std
end type
type dw_sel_numero from datawindow within w_factura_std
end type
end forward
...
integer width = 6162
integer height = 1924
string title = "Factura Standar 28.10.2016"
...
global w_factura_std w_factura_std
...
Do
		...
		If ll_plazo = 0 then
			...
			ls_leyenda = ls_leyenda + '~n' + ls_aux + ' ---> $ ' + string(ld_valor,"#,##0.00") &
			                 +' Plazo : '+string(ll_plazo)+' d$$HEX1$$ed00$$ENDHEX$$as'&
...
// Factura standar sin promociones
//select pr_valor
//into :li_promo
//from pr_param
//where em_codigo = :str_appgeninfo.empresa
...
//cedane , ferrocolor
//dw_report.DataObject = 'd_nventa_new'  //pinturasexpress  formato A5
...
//cedane 1550--- dw_factura_cedane_a4
//ferrocolor 1700
//motegi  1500
...
//dw_report.Modify("DataWindow.Footer.Height=1300")
...
//else
//	dw_report.modify("DataWindow.detail.Height=96")
//end if
...
public function integer wf_proforma_factura (long ai_factura);
string   ls_codant,ls_nombre,ls_codigo, ls_cliente,ls_motivo, ls_observ
...
declare c2 cursor for
 select it_codigo,dv_secue,dv_cantid,dv_precio,
        dv_desc1,dv_desc2,dv_desc3,dv_descue,dv_total,dv_motor,dv_chasis
   from fa_detve
  where em_codigo = 1
    and su_codigo = :str_appgeninfo.sucursal
    and es_codigo = 4
...
for li = 1 to ll_num_filas
	...
	if ls_valstk = 'S' then
		...
		If f_dame_stock_bodega_new(str_appgeninfo.seccion,str_detalle[li].item,lch_kit,lc) = false then
			...
			lc_valor= dw_detail.GetItemDecimal(li,"cc_subtotal")
...
integer li_ivaitem
decimal{2} lc_iva,lc_valor, ll_valorsum, ll_subtot,lc_valpag,lc_sum_tot,lc_basecero
string ls_val
...
// base tarifa cero
lc_basecero = dw_detail.getitemdecimal(1,"cc_totnoiva")


// actualiza el valor neto :  subtotal - descuento -basecero
ll_subtot= lc_sum_tot - lc_basecero - dw_master.GetItemdecimal(ll_filActMaster,"ve_descue")
...
// encuentra el valor a pagar : neto + iva + cargo +basecero
lc_valpag = ll_subtot + lc_iva + lc_basecero+dw_master.GetItemdecimal(ll_filActMaster,"ve_cargo")
...
this.Control[iCurrent+7]=this.dw_movim
this.Control[iCurrent+8]=this.dw_consulta_stk
this.Control[iCurrent+9]=this.p_producto
this.Control[iCurrent+10]=this.sle_1
this.Control[iCurrent+11]=this.cb_4
this.Control[iCurrent+12]=this.pb_1
this.Control[iCurrent+13]=this.st_1
this.Control[iCurrent+14]=this.st_plazo
this.Control[iCurrent+15]=this.st_2
this.Control[iCurrent+16]=this.cb_5
this.Control[iCurrent+17]=this.dw_1
this.Control[iCurrent+18]=this.dw_pedidos
this.Control[iCurrent+19]=this.dw_tinte
this.Control[iCurrent+20]=this.dw_detalle_pago
this.Control[iCurrent+21]=this.dw_sel_numero
end on

on w_factura_std.destroy
call super::destroy
...
if li_nreg = 0 then 
		messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","No se ha ingresado ningun producto...!!!",Exclamation!)
	return
end if
...
/*Recorrer nuevamente el detalle de la factura en busca de formulas de preparados para descargar los tintes de cada formula*/
...
for j = 1 to li_nreg
		...
		if isnull(ls_presentacion) or ls_presentacion = '' then 
			Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","La presentaci$$HEX1$$f300$$ENDHEX$$n del producto no ha sido definida...Por favor verifique....!!!",Exclamation!)
			return
		end if
		
		if lc_cantidad_x_preparar <= 0 then
			Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","La cantidad a preparar no puede ser negativa ni cero..Por favor verifique....!!!",Exclamation!)
			return
		end if
		...
		ll_fil  = f_tintes(ls_pepa,ls_formula,ls_presentacion,lc_cantidad_x_preparar,dw_tinte)
		 ...
		 //ingresar el movimiento de inventario en el kardex
          if ll_fil <= 0  then 
			Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","No existen tintes para procesar.!!!",Exclamation!)
			return
		end if	 
		  ...
		  for k = 1 to ll_fil
			  ...
			  lc_costo = dw_tinte.object.in_item_it_costo[k]
	           lc_cantidad_a_descargar = dw_tinte.object.cc_cantreq[k]
			
		      if lc_cantidad_a_descargar <= 0 then 
					Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","La cantidad a descargar no puede ser negativa ni cero...!!! ")
					return
			 end if
...
//						  
			  If wf_mov_inventario(ls_itcodtinte,lc_cantidad_a_descargar,hoy,ll_factura,is_estado,lc_costo,'N','',0,0,ls_codant,0,0) = FALSE Then
			  dw_movim.reset()
			  rollback;
...
type dw_master from w_sheet_master_detail`dw_master within w_factura_std
integer x = 0
integer y = 4
integer width = 6112
integer height = 708
...
string ls_hoy
decimal lc_subtotal= 0,lc_basecero = 0,lc_neto= 0
gb_codigo_numerico = true

//select to_char(sysdate,'yyyy') into :ls_hoy from  dual;
...
//if string(dw_master.getitemdatetime(ll_row,"ve_fecha"),'yyyy') = '2017' then
//	return 1
//end if

lc_subtotal = dw_detail.GetItemDecimal(ll_row,"cc_sum_subtot")
lc_basecero = dw_detail.GetItemDecimal(ll_row,"cc_totnoiva")
lc_neto = lc_subtotal - lc_basecero
dw_master.SetItem(ll_row,"ve_subtot",lc_subtotal)
dw_master.SetItem(ll_row,"ve_neto",lc_neto)
...
event dw_master::buttonclicked;call super::buttonclicked;Decimal lc_salcre,lc_cupcre
String ls_cliente,ls_estcre,ls_facturar
Integer ll_plazo
...
//
this.AcceptText()
ls_cliente = dw_master.GetItemString(row,'ce_codigo')


select ce_cupcre,ce_salcre,nvl(ce_facturar,'S')  ,ce_estcre,ce_plazo
into :lc_cupcre,:lc_salcre,:ls_facturar,:ls_estcre,:ll_plazo
from fa_clien
where em_codigo = :str_appgeninfo.empresa
and    ce_codigo= :ls_cliente;
...
type dw_detail from w_sheet_master_detail`dw_detail within w_factura_std
...
decimal{2} lc_prefab,lc_pedido, lc_stock,ld_null,lc_recargo,lc_preciodesc,lc_descprom
dec{4}     lc_costo
string    ls_columnName ,ls,ls_marca,ls_pepa, ls_nombre, ls_null, ls_codant,&
			  ls_valstk,tipo_descuento,ls_base,ls_param[2],ls_actividad,ls_itcodigo,ls_fp,ls_medida,lch_modifica,ls_presentacion
char       lch_cambia,lch_kit, lch_aux
decimal    ld_desc1, ld_desc2, ld_desc3
...
//	EXECUTE SP_ITEM;
//   FETCH SP_ITEM INTO :ls_marca,:li_ivaitem,:lch_cambia,:ls_pepa,:ls_codant,:ls_nombre,:lc_prefab, :ls_valstk, :ic_costo, :lch_kit, :ls_base,:ls_medida;
		
// con este voy a buscar primero por codigo anterior 
	select ma_codigo,it_tiemsec,it_preparado,it_codigo, it_nombre, it_prefab, it_valstk,nvl(it_costo,0),it_kit,it_base,ub_codigo,decode(ub_codigo,3,'CANECA',6,'GALON',7,'LITRO',8,'OCTAVO',9,'DIECISEIS',11,'TREINTADOS')
...
//		luego por codigo de barras
		 select ma_codigo,it_tiemsec,it_preparado,it_codigo, it_codant, it_nombre, it_prefab, it_valstk,nvl(it_costo,0),it_kit,it_base,ub_codigo,decode(ub_codigo,3,'CANECA',6,'GALON',7,'LITRO',8,'OCTAVO',9,'DIECISEIS',11,'TREINTADOS')
...
//filtrado para verificar si a este item le falta realizar el ajuste de egreso x preventa
	...
	if sqlca.sqlcode <> 0 then
		messageBox('Error Interno','No se encuentra producto en la sucursal: '+sqlca.sqlerrtext)
		return
	end if		
...
event dw_detail::buttonclicked;call super::buttonclicked;String ls_pepa,lch_modifica,ls_nulo
Decimal{2} lc_precio
...
type dw_report from w_sheet_master_detail`dw_report within w_factura_std
integer x = 105
integer y = 4
integer width = 2784
integer height = 732
