HA$PBExportHeader$w_um_enviar_transferencia.srw
$PBExportComments$/*Pendiente de  pruebar */
forward
global type w_um_enviar_transferencia from w_sheet_master_detail
end type
type dw_ubica from datawindow within w_um_enviar_transferencia
end type
type cb_1 from commandbutton within w_um_enviar_transferencia
end type
type dw_1 from datawindow within w_um_enviar_transferencia
end type
type dw_movim from datawindow within w_um_enviar_transferencia
end type
type dw_2 from datawindow within w_um_enviar_transferencia
end type
type sle_1 from singlelineedit within w_um_enviar_transferencia
end type
type st_6 from statictext within w_um_enviar_transferencia
end type
type cb_2 from commandbutton within w_um_enviar_transferencia
end type
type sle_2 from singlelineedit within w_um_enviar_transferencia
end type
type st_8 from statictext within w_um_enviar_transferencia
end type
type dw_4 from datawindow within w_um_enviar_transferencia
end type
type st_9 from statictext within w_um_enviar_transferencia
end type
type dw_vehiculos from uo_dw_basic within w_um_enviar_transferencia
end type
type dw_5 from datawindow within w_um_enviar_transferencia
end type
type st_1 from statictext within w_um_enviar_transferencia
end type
type ln_1 from line within w_um_enviar_transferencia
end type
end forward
...
global w_um_enviar_transferencia w_um_enviar_transferencia
...
public function integer wf_retrieve_mode ()
end prototypes
...
on w_um_enviar_transferencia.create
int iCurrent
call super::create
...
this.Control[iCurrent+13]=this.dw_vehiculos
this.Control[iCurrent+14]=this.dw_5
this.Control[iCurrent+15]=this.st_1
this.Control[iCurrent+16]=this.ln_1
end on

on w_um_enviar_transferencia.destroy
call super::destroy
...
type dw_master from w_sheet_master_detail`dw_master within w_um_enviar_transferencia
integer x = 0
integer y = 4
integer width = 2848
integer height = 460
...
type dw_detail from w_sheet_master_detail`dw_detail within w_um_enviar_transferencia
	...
	if sqlca.sqlcode <> 0 then
		...
		if lch_kit = 'V' then
			...
			if Message.StringParm = 'V' then
				...
				ls_ramv = mid(str_prodparam.chasis,pos(str_prodparam.chasis,'  RAMV: ')+8,pos(str_prodparam.chasis,'  A$$HEX1$$d100$$ENDHEX$$O: ') - (pos(str_prodparam.chasis,'  RAMV: ') + 8))
...
type dw_report from w_sheet_master_detail`dw_report within w_um_enviar_transferencia
integer x = 0
integer y = 28
integer width = 2697
integer height = 2112
...
long textcolor = 128
long backcolor = 67108864
string text = "N$$HEX1$$ba00$$ENDHEX$$. de Ticket:"
boolean focusrectangle = false
end type
...
long backcolor = 67108864
string text = "Transferencia de veh$$HEX1$$ed00$$ENDHEX$$culos "
boolean focusrectangle = false
end type
...
event doubleclicked;call super::doubleclicked;dw_5.visible = true
dw_5.insertrow(0)
...
ls_motor = data
if isnull(ls_motor) or trim(ls_motor) = "" then return
li_nfila = dw_vehiculos.find("di_ref1 like '%"+ls_motor+"'",1,li_reg)
if li_nfila > 0 then
	ldw_aux.scrolltorow(li_nfila)
	ldw_aux.setrow(li_nfila)
	ldw_aux.selectrow(li_nfila,true)
     ldw_aux.Object.seleccion[li_nfila] = 'S'
end if
end event
...
if dw_report.rowcount() = 0 then
	...
	if ll_nreg = 0 then
		messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","Debe ingresar antes los datos : Sucursal destino, bodega destino y n$$HEX1$$fa00$$ENDHEX$$mero del Ticket del Envio de transferencia")
		return
	end if
else
ll_nreg = dw_4.retrieve(str_appgeninfo.empresa,str_appgeninfo.sucursal,str_appgeninfo.seccion,ls_sucursal_destino,ls_bodega_destino,ls_ticket) 
