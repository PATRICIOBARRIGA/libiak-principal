HA$PBExportHeader$w_rep_max_min.srw
$PBExportComments$Si.reporte de m$$HEX1$$e100$$ENDHEX$$ximos y m$$HEX1$$ed00$$ENDHEX$$nimos con opciones de calificacion por parametros y sugeridos por rotaci$$HEX1$$f300$$ENDHEX$$n
forward
global type w_rep_max_min from w_sheet
end type
type dw_param from datawindow within w_rep_max_min
end type
type dw_subclases from datawindow within w_rep_max_min
end type
type dw_aux from datawindow within w_rep_max_min
end type
type dw_kits from datawindow within w_rep_max_min
end type
type dw_2 from datawindow within w_rep_max_min
end type
type dw_1 from datawindow within w_rep_max_min
end type
type dw_detalle from datawindow within w_rep_max_min
end type
type dw_ubica from datawindow within w_rep_max_min
end type
type dw_report from datawindow within w_rep_max_min
end type
type dw_datos from uo_dw_basic within w_rep_max_min
end type
type dw_item from datawindow within w_rep_max_min
end type
type dw_cabecera from datawindow within w_rep_max_min
end type
end forward
...
global w_rep_max_min w_rep_max_min

type variables
s_argInformation istr_argInformation
boolean ib_notAutoRetrieve
boolean ib_inReportMode,ib_flagpedido
String is_sucenvio, is_secenvio,is_columna,is_bodref
dec{2} ic_calta,ic_cmedia,ic_creg,ic_cbaja                 /*dias para stock cr$$HEX1$$ed00$$ENDHEX$$tico*/
dec{2} ic_ralta,ic_rmedia,ic_rregular,ic_rbaja             /*dias de rotacion*/
dec{2} ic_avgalta,ic_avgmedia,ic_avgreg,ic_avgbaja    /*promedios de ventas*/
dec{2} ic_eavgalta,ic_eavgmedia,ic_eavgreg,ic_eavgbaja /*promedios de ventas empresarial*/
...
/*Recuperamos parametros para calculos de sugerido*/
select  bo_codref,bo_ralta, bo_rmedia,bo_rregular,bo_rbaja, bo_palta, bo_pmedia,bo_pregular,bo_pbaja
into     :is_bodref,:ic_ralta,:ic_rmedia,:ic_rregular,:ic_rbaja,:ic_avgalta,:ic_avgmedia ,:ic_avgreg ,:ic_avgbaja
from   in_bode
...
ll_reg = dw_datos.retrieve(str_appgeninfo.empresa,str_appgeninfo.sucursal,str_appgeninfo.seccion,li_tipo,&
								...
								ls_codigo,ls_nombre,ls_clase,ls_fabricante,ic_ralta,ic_rmedia,ic_rregular,ic_rbaja,&
								                                                              ic_avgalta,ic_avgmedia,ic_avgreg,ic_avgbaja,&
      		 																		    is_bodref)
...
this.Control[iCurrent+2]=this.dw_subclases
this.Control[iCurrent+3]=this.dw_aux
this.Control[iCurrent+4]=this.dw_kits
this.Control[iCurrent+5]=this.dw_2
this.Control[iCurrent+6]=this.dw_1
this.Control[iCurrent+7]=this.dw_detalle
this.Control[iCurrent+8]=this.dw_ubica
this.Control[iCurrent+9]=this.dw_report
this.Control[iCurrent+10]=this.dw_datos
this.Control[iCurrent+11]=this.dw_item
this.Control[iCurrent+12]=this.dw_cabecera
end on

on w_rep_max_min.destroy
call super::destroy
...
select   bo_codref,bo_palta, bo_pmedia,bo_pregular,bo_pbaja, bo_ralta, bo_rmedia,bo_rregular,bo_rbaja,bo_calta,bo_cmedia,bo_cregular,bo_cbaja
into    :is_bodref,:ic_avgalta,:ic_avgmedia,:ic_avgreg,:ic_avgbaja,:ic_ralta,:ic_rmedia,:ic_rregular,:ic_rbaja,:ic_calta,:ic_cmedia,:ic_creg,:ic_cbaja
from   in_bode
where em_codigo = :str_appgeninfo.empresa
and     su_codigo = :str_appgeninfo.sucursal
and     bo_codigo = :str_appgeninfo.seccion;
...
dw_datos.resize(dw_cabecera.width - 420,li_height - dw_datos.y - dw_cabecera.y - 100)
this.setRedraw(True)
...
string title = "Ingreso de p$$HEX1$$e100$$ENDHEX$$rametros"
string dataobject = "d_opc_maxmin"
boolean controlmenu = true
boolean border = false
boolean livescroll = true
end type

event buttonclicked;Long ll_row,ll_nreg,i
DwObject  dwo_a,dwo_m,dwo_r,dwo_b
decimal{2} datos[],datos1[],datos2[],datos3[]

SetPointer(Hourglass!)
ll_row  = dw_datos.Getrow()
ll_nreg = dw_datos.rowcount()

/*1.- Ingreso de par$$HEX1$$e100$$ENDHEX$$metros para calificaci$$HEX1$$f300$$ENDHEX$$n de categor$$HEX1$$ed00$$ENDHEX$$as 
        El par$$HEX1$$e100$$ENDHEX$$metro a ingresar es el :promedio de ventas    */

if is_columna = 'calificacion' then
	
	dwo_a  = dw_datos.Object.avg_alta
	dwo_m = dw_datos.Object.avg_media
	dwo_r  = dw_datos.Object.avg_reg
	dwo_b  = dw_datos.Object.avg_baja
	...
	If dwo.name = 'b_1' then
		...
		accepttext()
		for i = 1 to ll_nreg
		datos[i]   =  getitemnumber(1,'p_alta')
		datos1[i] =  getitemnumber(1,'p_med')
		datos2[i] =  getitemnumber(1,'p_reg')
		datos3[i] =  getitemnumber(1,'p_baja')
		next
		
	end if
	
	/*Retorna los valores  PROMEDIOS por default  cuando se cancela la acci$$HEX1$$f300$$ENDHEX$$n*/
	if dwo.name = 'b_2' then
		if ll_row = 0 then return
		for i = 1 to ll_nreg
			datos[i]   =  ic_avgalta
			datos1[i] =  ic_avgmedia
			datos2[i] =  ic_avgreg
			datos3[i] =  ic_avgbaja
		next
		this.visible = false
	...
	dwo_m.object.primary  =  datos1
	dwo_r.object.primary    =  datos2
	dwo_b.object.primary   =  datos3
end if


/*2.-Numero de d$$HEX1$$ed00$$ENDHEX$$as que necesitamos que el producto rote*/	

if is_columna = 'sugerido' then

	/*Datos por default  nro de dias que se quiere que rote el inventario*/	
     /*Independiente de la sucursal*/

	
	dwo_a  = dw_datos.Object.r_alta
	dwo_m = dw_datos.Object.r_media
	dwo_r  = dw_datos.Object.r_reg
	dwo_b  = dw_datos.Object.r_baja
	...
	/*Retorna los valores por default para la cancelaci$$HEX1$$f300$$ENDHEX$$n*/
	if dwo.name = 'b_2' then
		if ll_row = 0 then return
		for i = 1 to ll_nreg
			datos[i]   =  ic_ralta
			datos1[i] =  ic_rmedia
			datos2[i] =  ic_rregular
			datos3[i] =  ic_rbaja
		next
		this.visible = false
	end if
...
integer height = 1588
integer taborder = 30
string dataobject = "d_rep_max_min_datos"
boolean hsplitscroll = true
...
messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","No existen datos para desplegar")
return
else
...
if dwo.name = 'codigo' then
	...
	if str_appgeninfo.sucursal = '90' then
		...
		dw_item.SettransObject(sqlca)
		dw_item.retrieve(str_appgeninfo.empresa,str_appgeninfo.sucursal,ls_pepa)
	end if	
...
/*Habilita la calificaci$$HEX1$$f300$$ENDHEX$$n de la rotaci$$HEX1$$f300$$ENDHEX$$n en base a par$$HEX1$$e100$$ENDHEX$$metros*/
if dwo.name = 'st_rotacion'  then 

is_columna = 'calificacion'
dw_param.reset()	
dw_param.modify("st_title.text = 'Ingrese PROMEDIOS de VENTAS para la calificaci$$HEX1$$f300$$ENDHEX$$n de items' st_param.text = 'Promedio (unid/dia)'")
dw_param.visible = true

dw_param.object.t_2.visible = true
dw_param.object.t_3.visible = true
dw_param.object.t_4.visible = true
dw_param.object.t_5.visible = true
dw_param.object.t_6.visible = false
dw_param.object.t_7.visible = false
dw_param.object.t_8.visible = false
dw_param.object.t_9.visible = false
dw_param.object.t_10.visible = false


dw_param.insertrow(0)
dw_param.Object.p_alta[1] = ic_avgalta
dw_param.Object.p_med[1]= ic_avgmedia
dw_param.Object.p_reg[1] = ic_avgreg
dw_param.Object.p_baja[1] = ic_avgbaja
end if

if dwo.name = 'st_sug'  then 
	
is_columna = 'sugerido'
dw_param.reset()	
dw_param.modify("st_title.text = 'Ingrese IR(indice rotaci$$HEX1$$f300$$ENDHEX$$n) para establecer un nuevo stock sugerido' st_param.text = 'IR ( N$$HEX2$$ba002000$$ENDHEX$$de veces)'")
dw_param.visible = true

dw_param.object.t_2.visible = false
dw_param.object.t_3.visible = false
dw_param.object.t_4.visible = false
dw_param.object.t_5.visible = false
dw_param.object.t_6.visible = true
dw_param.object.t_7.visible = true
dw_param.object.t_8.visible = true
dw_param.object.t_9.visible = true
dw_param.object.t_10.visible = true

dw_param.insertrow(0)
dw_param.Object.p_alta[1] = ic_ralta
dw_param.Object.p_med[1]= ic_rmedia
dw_param.Object.p_reg[1] = ic_rregular
dw_param.Object.p_baja[1] = ic_rbaja
end if
...
setnull(ls_filtro)
If dwo.name = 'b_1' Then
	dw_datos.object.cc_cantpedida.primary = dw_datos.object.cc_actual.primary
	dw_datos.object.cc_pedir.primary = dw_datos.object.cc_marca.primary
End if
...
If dwo.name = 'b_13' then
   ls_criterio = "(rotacion= 'ALTA' and dias <= "+string(ic_calta,'0')+" ) or (rotacion = 'MEDIA' and dias <= "+string(ic_cmedia,'0')+" ) or (rotacion= 'REGULAR' and dias <="+string(ic_creg,'0')+") or (rotacion = 'BAJA' and dias <= "+string(ic_cbaja,'0')+")"	
   dw_datos.Setfilter(ls_criterio)
   dw_datos.Filter()
   dw_datos.groupcalc()
end if	

If dwo.name = 'b_14' then
