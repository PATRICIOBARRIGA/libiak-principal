HA$PBExportHeader$w_compra_recepcion.srw
$PBExportComments$Pendiente.Si.Actualmente trabajando(10/01/07)Compras de productos de una empresa.Vale
forward
global type w_compra_recepcion from w_sheet_master_tabpage_detail
end type
type dw_compra from datawindow within w_compra_recepcion
end type
type dw_ubica from datawindow within w_compra_recepcion
end type
type dw_listarecep from datawindow within w_compra_recepcion
end type
type shl_1 from statichyperlink within w_compra_recepcion
end type
type shl_2 from statichyperlink within w_compra_recepcion
end type
type uo_cre from uo_cxp within w_compra_recepcion
end type
type dw_1 from datawindow within w_compra_recepcion
end type
type shl_3 from statichyperlink within w_compra_recepcion
end type
end forward
...
global w_compra_recepcion w_compra_recepcion
...
if ls = 'S' then
	ii_excento_iva = 0
     ...
     ic_iva = dw_master.GetItemdecimal(ll_filActMaster,'pctjeiva')
	 ii_excento_iva = 1
end if
...
this.Control[iCurrent+5]=this.shl_2
this.Control[iCurrent+6]=this.uo_cre
this.Control[iCurrent+7]=this.dw_1
this.Control[iCurrent+8]=this.shl_3
end on

on w_compra_recepcion.destroy
call super::destroy
...
//idw_m.setitem(ll_filact,"bo_codigo")

//idw_m.setitem(ll_filact,"co_numpad",il_nros[1])

for ll_i = 1 to idw_d.rowcount()
	idw_d.Object.co_numero[ll_i] = ll_nrocompra
	idw_d.Object.em_codigo[ll_i] = str_appgeninfo.empresa
	idw_d.Object.su_codigo[ll_i] = str_appgeninfo.sucursal
	idw_d.object.ec_codigo[ll_i] = is_estado
	idw_d.object.dc_secue[ll_i] = ll_i
Next
...
/*Validando que exista la cuenta por pagar*/
w_marco.setmicrohelp("Determinando si existe cuenta por pagar para esta factura...")
If wf_find_factura() = 0 Then
//w_marco.setmicrohelp("Cuenta por pagar no registrada...Por favor ingrese la factura...!!! ")
//return
End if
...
if ll_rc = 1 then
	  ...
	  if ll_rc = 1 then
									...
									DECLARE SP_INVMOV PROCEDURE FOR SP_INV_COMPRA_RECEPCION (:str_appgeninfo.empresa,:str_appgeninfo.sucursal,:str_appgeninfo.seccion,'1',:ll_nrocompra) ;
									EXECUTE SP_INVMOV;
									if SQLCA.SqlCode < 0 then
									Rollback;
									Messagebox("Atencion","Problemas al  Ejecutar en el SP_INV_COMPRA_RECEPCION"+ SQLCA.sqlerrtext)
									CLOSE SP_INVMOV;
									RETURN 
									else
									FETCH SP_INVMOV INTO :ll_rc;
									CLOSE SP_INVMOV;
									end if
...
if this.ib_inReportMode then
	...
	tab_detail.resize(li_width - 10 ,li_height -700)
	ldw_detail.resize(li_width - 100,li_height -900)
end if
this.setRedraw(True)
end event

type dw_master from w_sheet_master_tabpage_detail`dw_master within w_compra_recepcion
integer width = 4352
integer height = 680
...
//	   where em_codigo = :str_appgeninfo.empresa
//		 and pr_nombre = 'IVA';
		ic_iva = dw_master.GetItemDecimal(row,'pctjeiva')
 		ii_excento_iva = 1
	end if
...
//		end if	
//    end if 
    lc_ivavalor = lc_subtot*ic_iva
	this.Object.co_iva[row] = lc_ivavalor
	lc_transporte = this.Object.co_transpor[row]
	lc_valortrf0    = this.Object.co_subtrf0[row]
...
type dw_report from w_sheet_master_tabpage_detail`dw_report within w_compra_recepcion
integer x = 2510
integer y = 1196
string dataobject = "d_rep_compra"
end type

type tab_detail from w_sheet_master_tabpage_detail`tab_detail within w_compra_recepcion
integer x = 14
integer y = 708
integer width = 4329
integer height = 1604
...
Decimal{4}  lc_indie,lc_newprecio,lc_descab,lc_factor,lc_costo,&
...
event dw_detail1::itemchanged;call super::itemchanged;long ll_filact,ll_itemact,ll_fila,ll_filActMaster,ll_totFilas,li_pagaiva
decimal ll_valortot,ll_valor,ll_costo,ll_valorsum,ld_area, ll_ivavalor, lc_transporte
...
//		end if
		...
		/*El nuevo precio no debe depender de la caracter$$HEX1$$ed00$$ENDHEX$$stisca del proveedor 		a todos se le debe a$$HEX1$$f100$$ENDHEX$$adir el 5% virtual*/
...
string text = "Registrar cr$$HEX1$$e900$$ENDHEX$$dito"
boolean focusrectangle = false
end type

event clicked;uo_cre.visible = true
uo_cre.bringtotop = true


uo_cre.dw_datos.enabled = false
uo_cre.dw_2.enabled       = false
uo_cre.dw_3.enabled       = false

//uo_cre.dw_datos.reset()
//uo_cre.dw_a.reset()
//uo_cre.dw_2.reset()
//uo_cre.dw_3.reset()
//uo_cre.dw_c.reset()
//
//
///*Asignar los datos de la compra a la cxp */
Long ll_row,ll_new,ll_numrecep,ll_reg
//Integer li_codfte,li_codiva
//Date ld_fecha,ld_fpago,ld_fecrecep
String ls_fpcodigo,ls_pvcodigo,ls_codretfte,ls_codretiva,ls_pctjeret,ls_cofacpro
//Real lr_plazo
//Decimal{2} lc_pctjeret
//
ll_row  = dw_master.getrow()
//
////Load variables
...
ls_pvcodigo = dw_master.Object.pv_codigo[ll_row] 
//ll_numrecep = dw_master.Object.co_numpad[ll_row]
ls_cofacpro = dw_master.Object.co_facpro[ll_row]
//
////recuperar la cxp y habilitar datawindows
ll_reg = uo_cre.dw_datos.retrieve(str_appgeninfo.empresa,ls_pvcodigo,ls_cofacpro)
if ll_reg = 0 then
	Messagebox("Atenci$$HEX1$$f300$$ENDHEX$$n","No se ha registrado a$$HEX1$$fa00$$ENDHEX$$n la cuenta por pagar...Por favor verifique...!!!")
//	uo_cre.dw_datos.enabled = false
//	uo_cre.dw_2.enabled       = false
//	uo_cre.dw_3.enabled       = false
//	return 
...
//
////si no existe cxp entonces cargar por default 
//
//ll_new = uo_cre.dw_datos.insertrow(0)
//
///*Para determinar la fecha de pago*/
//SELECT TO_DATE(CO_FECHA)
//INTO   :ld_fecrecep
...
//
///*Cargamos por default los valores predeterminados de retenci$$HEX1$$f300$$ENDHEX$$n asignados al proveedor */
//SELECT  F.RF_TIPO, F.RF_PROCEN
//INTO     :ls_codretfte,:lc_pctjeret
//FROM    IN_PROVE P,CC_RETEN  F 
//WHERE   F.RF_CODIGO = P.RF_CODIGO
//AND       P. PV_CODIGO = :ls_pvcodigo;
//
//uo_cre.dw_datos.Object.tv_codigo[ll_new] = '1'  // Factura de compra
//uo_cre.dw_datos.Object.pv_codigo[ll_new] = dw_master.Object.pv_codigo[ll_row]
//uo_cre.dw_datos.Object.co_facpro[ll_new] = dw_master.Object.co_facpro[ll_row]
//uo_cre.dw_datos.Object.mp_preimp[ll_new] = dw_master.Object.co_facpro[ll_row]
//uo_cre.dw_datos.Object.co_numero[ll_new] = dw_master.Object.co_numero[ll_row]
//uo_cre.dw_datos.Object.mp_fecpago[ll_new] = ld_fpago
//
//
///*Asignamos por default el % de retenci$$HEX1$$f300$$ENDHEX$$n definido en el proveedor*/
//if isnull(ls_codretfte) or ls_codretfte = "" then ls_codretfte = '332'
//
//ll_new =       uo_cre.dw_2.insertrow(0)
//uo_cre.dw_2.Object.cp_pago_pm_codpct[ll_new] = ls_codretfte
//uo_cre.dw_2.Object.cp_pago_pm_base0[ll_new] = 0
//uo_cre.dw_2.Object.cp_pago_pm_basegrav[ll_new] =   round(dw_master.Object.co_subtot[ll_row],2)
//uo_cre.dw_2.Object.cp_pago_pm_basenograv[ll_new] = round(dw_master.Object.co_subtrf0[ll_row],2)
//uo_cre.dw_2.object.cp_pago_pm_porctje[ll_new] = lc_pctjeret
//uo_cre.wf_calculo_retencion(uo_cre.dw_2)
//uo_cre.dw_2.object.cp_pago_pm_valor.primary = uo_cre.dw_2.object.cc_retfte.primary
//
//
//
///*Asignamos por default el % de retenci$$HEX1$$f300$$ENDHEX$$n de iva definido en el proveedor*/
//SELECT  F.RF_TIPO,F.RF_PROCEN
//INTO     :ls_codretiva,:lc_pctjeret
//FROM    IN_PROVE P,CC_RETEN  F 
//WHERE  F.RF_CODIGO = P.RF_CODIGO2
//AND       P. PV_CODIGO = :ls_pvcodigo;
//
//if not isnull(ls_codretiva) and ls_codretiva <> "" then
//ll_new =  uo_cre.dw_3.insertrow(0)
//uo_cre.dw_3.Object.cp_pago_pm_codpct[ll_new]  = ls_codretiva
//uo_cre.dw_3.Object.cp_pago_pm_bseimp[ll_new] = round(dw_master.Object.co_iva[ll_row],2)
//uo_cre.dw_3.object.cp_pago_pm_porctje[ll_new]  = lc_pctjeret
//uo_cre.dw_3.object.cp_pago_pm_valor.primary     = uo_cre.dw_2.object.cc_retfte.primary
//end if
//End if
end event
...
event clicked;uo_cre.visible = false
uo_cre.bringtotop = false
end event
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_importa_items_compra_recepcion"
boolean border = false
boolean livescroll = true
end type
...
event clicked;String null_str,ls_codant,ls_codigo,ls_nombre,lch_kit
SetNull(null_str)
long ll_reg,ll_i,ll_cantid,li_pagaiva
dec{4} lc_costo,lc_precio


str_prodparam.ventana = parent
str_prodparam.datawindow = idw_d
str_prodparam.fila = idw_d.GetRow() 
...
cantidad_producto_ubica = TRUE
	...
	idw_d.setredraw(false) 
    for ll_i = 1 to ll_reg
 
			idw_d.insertrow(0)
						
			ls_codant = dw_1.getitemstring(ll_i,"codant")
			ll_cantid = dw_1.getItemdecimal(ll_i,"cantidad")
			lc_costo  = dw_1.getitemDecimal(ll_i,"costo")
...
//			end if	
			...
			select it_codigo,it_nombre,it_kit,it_tiemsec,it_prefab
			into :ls_codigo,:ls_nombre,:lch_kit,:li_pagaiva,:lc_precio
			from in_item
			where em_codigo = :str_appgeninfo.empresa
			and it_codant = :ls_codant;
			if sqlca.sqlcode <> 0 then
				messagebox("Error...verifique","No existe c$$HEX1$$f300$$ENDHEX$$digo del item: "+ls_codant,stopsign!)
				return
			end if
						
			idw_d.setitem(ll_i,"em_codigo",str_appgeninfo.empresa)
			idw_d.setitem(ll_i,"su_codigo",str_appgeninfo.sucursal)
			idw_d.setitem(ll_i,"bo_codigo",str_appgeninfo.seccion)
			
			idw_d.setitem(ll_i,"dc_secue",ll_i)		
						
			idw_d.setitem(ll_i,"it_codigo",ls_codigo)
			idw_d.setitem(ll_i,"codant",ls_codant)
			idw_d.setitem(ll_i,"nombre",ls_nombre)
						
			idw_d.setitem(ll_i,"dc_cantid",ll_cantid)
       		idw_d.setitem(ll_i, 'it_kit', lch_kit)
			idw_d.setitem(ll_i,'it_tiemsec',li_pagaiva)	 
			idw_d.setitem(ll_i,"dc_costo" ,lc_costo)
			idw_d.setitem(ll_i,'dc_subtot',lc_costo)
			idw_d.setitem(ll_i,'dc_preact',lc_precio)
			idw_d.SetItem(ll_i,"dc_total",idw_d.GetItemDecimal(ll_i,"cc_total"))

			
    next	
wf_actualiza_suma()	 
idw_d.setredraw(true)
end event
